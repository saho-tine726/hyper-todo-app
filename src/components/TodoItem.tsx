...
type Props = {
  ...
  getTodoBgColor: (deadline: string, status: string, today: string) => string;
  getTodoColor: (deadline: string, status: string, today: string) => string;
...
export const TodoItem: React.FC<Props> = (props) => {
  ...
  return (
    <GridItem bg={getTodoBgColor(todoItem.deadline, todoItem.status, today)} boxShadow="md" display="flex" flexDirection="column" h="100%" justifyContent="space-between" key={todoItem.id} maxW={{ base: "100%", md: "362px" }}>
